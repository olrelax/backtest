...
def numerate(df, i):
    return df.rename(
        columns={'strike': 'strike_%d' % i, 'overstrike': 'overstrike_%d' % i, 'under_enter': 'under_enter_%d' % i,
                 'under_out': 'under_out_%d' % i,
...
def decode_algo(algo_str):
    ...
    for i in range(count):
        ...
        if algo_names[i] in ('op', 'otm_prc'):
            ...
            exit('Unknown algo_name <%s>. Choose one:\nop|otm_prc, ou|otm_usd, hu|hedge_usd,fp|fix[ed]_price' %
                 algo_names[i])
...
def in2exp(side, opt_type, i):
    ...
    df = df.rename(columns={enter_price: 'enter', under_enter_price: 'under_enter', 'close': 'out_tmp',
                            'underlying_close': 'under_out'})
    ...
    if monday_only:
        ...
        condition = 0
        df['k'] = pd.Series(map(greater_than_condition,
                                df['overstrike'])).to_frame()  # make out price 0.0 if not executed on expiration
    ...
    #        save(df,'df_mo',True)
        ...
        df_out = df.loc[df['quote_date'] == df['expiration']][
            ['expiration', 'strike', 'under_out', 'out_tmp']].copy().reset_index(drop=True)
        df_out['overstrike'] = (df_out['strike'] - df_out['under_out']) * sign
        condition = 0
        df_out['k'] = pd.Series(map(greater_than_condition, df_out[
            'overstrike'])).to_frame()  # make out price 0.0 if not executed on expiration
...
def main_proc():
    ...
    sum_fmt = 'profit %.2f' % summa if capital in ('', 'per opt') else 'profit %.2f%%' % summa if capital in (
    '%', 'prc') else 'profit/capital $%.2f/$%.2f' % (summa, float(capital))
