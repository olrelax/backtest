...
from params import premium_limit
...
def get_in2exp(param, side, opt_type, i):
    ...
    o_out = o_out[['expiration', 'strike', 'underlying_bid_1545', 'underlying_ask_1545', 'bid_1545', 'ask_1545',
    ...
    o = o.rename(columns={'strike': 'strike_%d' % i, 'underlying_bid_1545': 'under_bid_in_%d' % i,
                          ...
                          'margin': 'margin_%d' % i})
    o_loss = get_strike_loss(o, i, 'under_bid_1545_out') if side == 'S' and not strike_loss_limit == '' else ''
    o = o.drop(columns={'under_bid_1545_out_%d' % i, 'under_ask_1545_out_%d' % i, 'bid_1545_out_%d' % i,
                        'ask_1545_out_%d' % i})
    if len(o_loss) > 0:
        o_loss = o_loss.drop(
            columns={'under_bid_out_%d' % i, 'under_ask_out_%d' % i, 'bid_out_%d' % i, 'ask_out_%d' % i})
        o_loss = o_loss.rename(columns={'under_bid_1545_out_%d' % i: 'under_bid_out_%d' % i,
    ...
    o['exit_date'] = o['expiration']
    cols = o.columns.tolist()
    cols.insert(2, 'exit_date')
    cols.pop(-1)
    o['enter_date'] = o['quote_date']
